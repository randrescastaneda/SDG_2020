...
---
.left-column[
.big[It is relatively easy to see that the distribution of Findland is more equalitarian than the one in Costa Rica. we can see several things
...
---
.left-column[
.big[However, this kind of comparisons are only useful with population of similar sizes. When we include a country with larger population this happens. 
]
]
.right-column[
```{r pe1}
...
---
.left-column[
.big[We look the capacity to compare distributions because the population of Colombia is almost ten times higher than the one in Findland and Costa Rica.  
]
]
.right-column[
```{r pe1}
