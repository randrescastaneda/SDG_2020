...
.big[Unfortunately, there is no data available for a representative set of countries in any year to build a comparable trend of global inequality over time]
...
## So, inequality is decreasing in LAC and in some other countries, but it is also increasing in others. Can we know something globally?  
...
.left-column[
Take for instance `r dfc_rep[["countryname"]][1]`, whose 90th percentile (`r sp90[1]`) is `r sr9010[1]` higher than its 10th percentile (`r sp10[1]`). 
]
.right-column[
```{r p1090gl2}
p_p10p902
...
---
.left-column[
The most remarcable change is by `r names(cc1)[1]`
]
.right-column[
```{r g0095-2}
p_0095_2
...
---
.left-column[
The most remarcable change is by `r names(cc1)[1]`, followed by `r names(cc1)[2]`
]
.right-column[
```{r g0095-3}
p_0095_3
...
---
.left-column[
The most remarcable change is by `r names(cc1)[1]`, followed by `r names(cc1)[2]` and `r add_and(names(cc1)[3:5])`
]
.right-column[
```{r g0095-4}
p_0095_4
...
## What if we remove the tails? say the bottom 10 and the top10
---
